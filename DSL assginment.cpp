...
//For representation of node in adjacency list
struct node
{
  int data;
  struct node *link;
};
//Show connected vertexes
struct vertexlist
{
  struct node *vlisthead;
};

//graph connections to other vertexes.
struct Graph
{
  int v;
...
// Declaring the graph according to the number of vertex.
struct Graph *
CreateGraph (int n)
{
  int i;
  ...
  // declaring list for n number of vertex.
...
// To add the edge into the undirected graph.
void
InsertNode (Graph * G, int v1, int v2)
{
